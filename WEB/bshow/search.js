function searchList() {
  ...
  // Select show when pressing Enter key
  $('.search').on('keydown', function (event) {
    if (event.keyCode === 13) {
      ...
      if (selectedResult) {
        ...
        let firstResult = $('#search-list li:first-child');
        if (firstResult.length) {
          window.location.href = firstResult.find('a').attr('href');
        }
      }
    }
  });
  ...
  $(document).on('click', function (event) {
    if (!$(event.target).is('.search, #search-list *')) {
      $('li').hide();
      $('.search').val('');
    }
  });
